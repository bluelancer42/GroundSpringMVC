 ...
 * 
 * import com.ground.dao.EmployeeDao; import com.ground.model.Employee;
 * 
 * @Repository public class EmployeeDaoImpl extends JdbcDaoSupport implements
 * EmployeeDao{
 * 
 ...
 * 
 * @Override public void insertEmployee(Employee emp) { String sql =
 * "INSERT INTO employee " + "(empId, empName) VALUES (?, ?)" ;
 * getJdbcTemplate().update(sql, new Object[]{ emp.getEmpId(), emp.getEmpName()
 * }); }
 * 
 * @Override public void insertEmployees(final List<Employee> employees) {
 ...
 * }
 * 
 * @Override public List<Employee> getAllEmployees(){ String sql =
 * "SELECT * FROM employee"; List<Map<String, Object>> rows =
 * getJdbcTemplate().queryForList(sql);
 * 
 * List<Employee> result = new ArrayList<Employee>(); for(Map<String, Object>
 * row:rows){ Employee emp = new Employee();
 * emp.setEmpId((String)row.get("empId"));
 * emp.setEmpName((String)row.get("empName")); result.add(emp); }
 * 
 * return result; }
 * 
 ...
 * emp.setEmpId(rs.getString("empId")); emp.setEmpName(rs.getString("empName"));
 * return emp; } }); } }
 */